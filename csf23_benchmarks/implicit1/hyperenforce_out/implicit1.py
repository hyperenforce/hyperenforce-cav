...
# ucon_x = false
# supp_0 = false
plant.add_vertex(label=pt(is_cont="0", PC="5", con_y="false")) # index:3
...
# ucon_x = true
# supp_0 = false
# repeating state at PC=5, goes back to index:3
...
wait_outputs = lambda v1, v2: NOT(OR(pt_enc.encodes_var_form(v1, PC='13'),pt_enc.encodes_var_form(v2, PC='213')))
...
def _run_cont_synth(plant, h):
    prob = ControllerSynthesisEncodingQBF(plant, h, do_optimization=True, no_deadlocks=False)
    prob.encode()
    output = prob.solve()
    if output.is_error:
