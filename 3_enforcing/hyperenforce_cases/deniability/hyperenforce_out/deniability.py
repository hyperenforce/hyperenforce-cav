...
pt = DataType({'is_cont': ['0', '1', 'null', ''], 'PC': ['4', '13', '21', '23', '25', '27', 'null', '']})
...
plant.add_vertex(label=pt(is_cont="1", PC="13")) # index:2
plant.add_cont_edges([(1, 2)], weight=[1])
plant.add_vertex(label=pt(is_cont="0", PC="21")) # index:3
plant.add_uncont_edges([(2, 3)], weight=[1])
plant.add_vertex(label=pt(is_cont="0", PC="23")) # index:4
plant.add_uncont_edges([(3, 4)], weight=[1])
plant.add_vertex(label=pt(is_cont="0", PC="25")) # index:5
plant.add_uncont_edges([(4, 5)], weight=[1])
plant.add_vertex(label=pt(is_cont="1", PC="27")) # index:6
...
true_form = lambda v1, v2, v3: AND(AND(pt_enc.same_var_form(v1, v2, 'con_num_l_obs')), AND(pt_enc.same_var_form(v1, v3, 'con_num_l_obs')))
wait_outputs = lambda v1, v2, v3: NOT(OR(pt_enc.encodes_var_form(v1, PC='27'),pt_enc.encodes_var_form(v2, PC='27')))
end_condition = lambda v1, v2, v3: AND(NOT(pt_enc.same_var_form(v2, v3, 'ucon_num_h_balance')), AND(pt_enc.encodes_var_form(v1, PC='27'),pt_enc.encodes_var_form(v2, PC='27')))
